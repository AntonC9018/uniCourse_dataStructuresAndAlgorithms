...
- **Barbershop drones.** The workers are now automatic drones:
  several drones may process the same client at the same time,
  each cutting 1 minute per tick.
  When a client is fully cut, every drone on that client becomes free.
  Simulate until all clients are done.
  ...
  Take the structs, `add_client`, `take_first_waiting`, `cut_one_minute`,
  `client_done`, and `shop_done` from the previous task as is —
  only the rules change.
  A client is no longer exclusive: an idle drone with nobody waiting
  helps whoever is already being cut.
  Sharing is safe because every drone cuts through a pointer into the same client.

  Finding the first client already being cut is its own function,
  next to `take_first_waiting`:
  ...
  void assign_drones(Shop& shop)
  {
      for (auto& worker : shop.workers)
      {
          ...
          worker.assignedClient = next; // stays idle when there is nobody at all
      }
  }
  ```

  One drone tick is unchanged; the shop tick cuts with every drone first
  and only then releases whoever is done — from every drone at once.
  Cutting twice in one tick is the whole point,
  and the `value > 0` guard keeps a shared last minute from going negative:
  ...
  Say alice needs 4 minutes and both drones take her:
  tick 1 cuts twice (4 to 2), tick 2 cuts twice (2 to 0),
  both drones report `Done` and are released together:
...
- **Amusement park.** A park has 3 attractions and a waiting line for 4 visitors.
  Each visitor ranks all 3 attractions by preference, remembers the last one visited,
  and may visit at most 3 attractions in total.
  Every tick finished visitors go back to the line —
  or leave the park when their quota is used up —
  and waiting visitors take the first free attraction from their preferences,
  other than the one they just left.
  Simulate until everyone has used up their quota.
  ...
  Nobody owns anybody here either: attractions, visitors, and the park
  all live in `main`, and every link between them is a pointer.
  Attractions point at their current occupants (occupancy changes every tick,
  so a copy would strand stale state the moment someone is released),
  while visitors point at the attractions they wish for (stable wishes,
  set once in `main`) plus the last one visited.
  Releasing someone is just pointing somewhere else:
  back into the search queue, or nowhere when the quota is done.
  ...
  struct Visitor; // an attraction only borrows its occupant
  ...
  {
      std::string_view label;
      std::array<Attraction*, 3> preferences; // most wanted first, each listed once
  ...
  Adding to the line mirrors the shop queue.
  The visitor must never be `nullptr`, so it is asserted;
  a full line is reported through the result:

  ```cpp
  AddVisitorResult add_visitor(Park& park, Visitor* visitor)
  {
      assert(visitor != nullptr); // add_visitor expects visitor to never be null
  ...
  Picking is its own function: the first preference that is free
  and is not the one just left.
  Preferences are set in `main` and always valid, so a null there is asserted:
  ...
  void board_visitors(Park& park)
  {
      for (auto& slot : park.waiting)
      {
          ...
          if (choice == nullptr)
          {
              continue; // everything wanted is busy: keep waiting
          }
  ...
  One tick finishes every ongoing ride — a visit takes exactly one tick —
  then seats whoever is waiting.
  A finished visitor with visits left goes back to the line;
  one with a used-up quota simply leaves (its pointer is dropped).
  Releasing can never overflow the line:
  at most as many visitors come back as were riding:
  ...
  void finish_rides(Park& park)
  {
      for (Attraction* attraction : park.attractions)
      {
          ...
          {
              continue; // quota used up: out of the park
          }
  ...
  Say alice wants the coaster first and bob wants the wheel first:
  tick 1 seats both (each gets their favorite),
  tick 2 finishes both rides (one visit counted each) and seats them again —
  this time each takes whatever is free past the one just left:
...
- **Family hugs.** A family has 5 people.
  ...
  Five people means four hugs each, that is ten pairs in total (5 * 4 / 2).
  Notice that hugging needs no container at all:
  two people and their pointer lists are enough,
  because a pointer already says exactly who was hugged:
  ...
  Pairing every `i` with every later `j` visits each pair exactly once.
  The family array never moves, so the stored pointers stay valid:
...
- **Family hugs with a bit set.** Same 5 people,
  but each person stores a `std::bitset<5>` of the members they hugged.
  (Yes, C++ has a standard bit set: `<bitset>`.)
  Hug everyone once and count the hugs.
  ...
  Bit `i` means "hugged member `i`".
  Your own bit is never set, so a fully hugged person has 4 bits out of 5.
  Same ten pairs as before — only the storage changes:
