...
и записать результат (84) обратно по адресу 102.
...
Теперь каждая из этих инструкций имеет *идентификатор*. 
...
Для простоты предположим, что идентификатор чтения из основной памяти равен 15,
...
1. Использовать инструкцию с идентификатором 15, где первый аргумент (адрес основной памяти) равен 102, а второй аргумент (адрес процессорной памяти) - 0. 
...
то можем отличить ее от другой инструкции копирования 15.
...
```
Вот как процессор выполнит этот код: он прочитает первое число как идентификатор инструкции (15).
...
Поэтому он считывает 102, считывает 0, а затем выполняет инструкцию с идентификатором 15,
