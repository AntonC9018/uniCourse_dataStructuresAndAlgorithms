...
В отличие от обычного `int*`, этот тип не позволяет `nullptr` как обычное пользовательское значение.

Исключение — объект, из которого значение было перемещено. Move-конструктор или move-оператор
присваивания может оставить такой объект с `heapValue == nullptr`, чтобы прежняя память
не была освобождена дважды. Перемещённый объект по-прежнему должен оставаться корректным для
уничтожения и повторного присваивания, но считывать его значение до нового присваивания нельзя.
...
12. В какой еще ситуации вызовется rvalue reference перегрузка конструктора или оператора присваивания?
    ...
    *Перегрузка* означает функция с тем же именем, но другими параметрами.
    </details>
...
14. Объясните, какая по вашему мнению была мотивация того, что у этого типа не был определен дефолтный конструктор?
    ...
    *Контракт класса* означает что-то типа правил, которые всегда выполняются при существовании объекта,
    вне зависимости от того, какие операции с ним были выполнены.

    Для этого класса контракт заключается в том, что `heapValue` не может быть равен `nullptr`,
    кроме явно оговоренного состояния объекта после перемещения.
