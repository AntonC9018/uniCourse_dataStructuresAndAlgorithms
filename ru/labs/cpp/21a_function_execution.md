...
| sp - 8 | sp - 4                            | sp |
| -----  | ------                            | -- |
| x (1)  | адрес возврата к `return 0;` из `main` | ?? |

3. На `return;` из `f`

| sp - 8 | sp - 4                            | sp |
| -----  | ------                            | -- |
| x (0)  | адрес возврата к `return 0;` из `main` | ?? |

4. На `return 0;` из `main`

| sp - 4 | sp |
| -----  | ------                            |
| x (0)  | адрес возврата из `main` |

</details>

`sp - 8` при выполнении `f` ссылается на адрес локальной переменной `x` из `main`.
...
`sp - 8  -->  sp - 4`, что равно адресу возврата.
...
> 🤓 Раскладка стека в таблицах выше — концептуальное упрощение: что именно попадает в кадр стека
> (локальные переменные, адрес возврата, сохраненные регистры) и в каком порядке, решает реализация,
> стандарт этого не предписывает.
> На практике же на любом современном железе со стандартной раскладкой стека служебные данные кадра —
> включая адрес возврата — действительно лежат рядом с локальными переменными,
> поэтому идея упражнения выше жизнеспособна.
