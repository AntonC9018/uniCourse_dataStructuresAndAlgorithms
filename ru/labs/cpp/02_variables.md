...
- [Видео по основам, более углубленная информация (до указателей)](https://www.youtube.com/watch?v=9AhNOjjyAwU&list=PL4sUOB8DjVlWUcSaCu0xPcK7rYeRwGpl7&index=14)
- [Видео про конверсии типов (до указателей)](https://www.youtube.com/watch?v=imkDyDE7Tkw&list=PL4sUOB8DjVlWUcSaCu0xPcK7rYeRwGpl7&index=28)
...
Это запрещено, даже если второе определение другого типа:
```cpp
int a = 5;
...
<details>
<summary>Что здесь считается выражением?</summary>

Литерал (`5`, `6`, `7`), чтение переменной (`b`) и даже целое присваивание (`a = b`) — все это выражения.
Каждое из них вычисляется в одно значение со своим типом.
Быть выражением значит, что само присваивание дает присвоенное значение,
поэтому его можно использовать там, где ожидается значение — например, `int c = (a = b);`
сначала впишет `b` в `a`, а затем то же самое значение впишет в `c`.
</details>
...
При выполнении программы он не работает: компилятор заменяет `sizeof(...)` на обычное число,
так что `sizeof` как функции при выполнении вообще не существует.
...
В `sizeof(a + 1)` выражение `a + 1` не выполняется;
компилятор смотрит только на тип, который был бы у его результата (здесь `int`),
потому что типы известны только при компиляции и до выполнения программы не доживают.
...
<details>
<summary>Что значит <code>auto</code>?</summary>

`auto` значит, что тип автоматически выводится из типа выражения, которым инициализируют,
...
Выведенный тип статичный (фиксируется при компиляции) и поменяться позже не может:
```cpp
auto a = 5;
a = "abc"; // ошибка: `a` это `int`, строкой позже он стать не может
```
</details>

### 10. `auto` без инициализации
```cpp
auto a;
a = 5;
```
...
Присваивание `5` на следующей строке не чинит это —
тип должен быть зафиксирован при определении и не может быть выведен задним числом из позднего присваивания.
</details>

### 11. `auto` от другой переменной
```cpp
int a{ 5 };
auto b{ a + 5 };
```
...
`auto` смотрит на тип всего выражения, а не только на то, литерал это или нет.
У `a + 5` тип `int`, поэтому `b` станет `int` со значением `10`.
То же верно для любого выражения: `auto c{ a };` тоже даст `c` тип `int`.
</details>

### 12. `auto` с универсальной инициализацией
...
<details>
<summary>Что делает <code>static_cast&lt;uint8_t&gt;</code>?</summary>

`static_cast<uint8_t>(5)` это выражение, 
результатом которого является число `5` типа `uint8_t`.
...
- Выполняется `static_cast` в тип, указанный внутри `<...>`, то есть в `uint8_t`.
  Поскольку `5` влезает в 1 байт, конвертируется без проблем.
...
### 16. Побитовая операция (продвинутый уровень)
```cpp
#include <cstdint>
#include <iostream>
...
### 17. Смена знака (продвинутый уровень)
```cpp
#include <cstdint>
#include <iostream>
