...
<details>
<summary>Что делает <code>std::endl</code>?</summary>

`endl` означает "end line" (`end` + `line`, конец строки).
Он завершает текущую строку вывода (как нажатие Enter)
и сбрасывает буфер вывода, чтобы вывод сразу появился на экране.
</details>
...
Произойдет ошибка компиляции, потому что C++ чувствителен к заглавности букв:
`cout` нужно писать маленькими буквами.
`COUT` большими буквами это другое имя, а такого имени в `std` нет.
...
### 14. `main` вызывает функцию вызывает функцию
```cpp
#include <iostream>
