...
Здесь `sizeof(arr)` — это 16 (4 инта по 4 байта),
`sizeof(arr[0])` — это 4 (один `int`),
поэтому `16 / 4` даст 4 элемента.
...
### 11. Использование массива как адрес
```cpp
int arr[2]{};
int* p = arr;
*arr = 1;
...
Когда `arr` используется в качестве выражения в `std::cout`,
он деградирует в указатель на первый элемент,
то есть это эквивалентно `&arr[0]`.

Напечатается этот адрес, а не элементы `1`, `2`, `3`.
