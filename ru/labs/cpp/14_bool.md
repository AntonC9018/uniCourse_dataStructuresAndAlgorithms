...
### 18. Функции, которые никогда не вызываются
```cpp
#include <iostream>
...
Определение функций `A` и `B` лишь описывает, что *произошло бы* при их вызове.
Поскольку `main` не вызывает ни одной из функций, их тела никогда не выполнятся,
поэтому ничего не напечатается.
</details>

### 19. Тонкости оператора `&&` (1)
```cpp
#include <iostream>
...
### 23. Функция перезаписывает переменную из выражения параметра
```cpp
#include <iostream>
