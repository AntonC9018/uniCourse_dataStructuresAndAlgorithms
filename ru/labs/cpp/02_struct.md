...
1. ```cpp
   ...
   На строчке `int b { func(1) };` происходит 3 вещи:
   - выделяется память для переменной `b`;
   - высчитывается результат выражения `func(1)`;
   ...
   **Результат выражения** присваивается к `b`!
   ...
   <details>
   <summary>Какой тип у результата выражения <code>func(1)</code>?</summary>
   ...
   Так как в `b` на строчке `int b { func(1) }` попадет результат
   вызова функции, в `b` попадает `1` после всех вышеописанных операций.
   </details>
