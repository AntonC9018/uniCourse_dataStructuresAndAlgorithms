...
Сложность заключается в расположении элементов в памяти. 
...
Также существует метод оптимизации Dope Vector, который позволяет ускорить вычисление линейного индекса.
...
Идея в том, что мы можем представить, будто все элементы в группах по 3, но они же находятся линейно в памяти.
...
Один из подходов будет найти сначала индекс группы по 3 -- это будет индекс ряда `a`, раз по 3 группы в каждом ряду, + `b`.
...
Dope Vector означает все значения $` D_j `$ и значение $` c `$.
