...
> Заметьте, это не касается динамической памяти: `new` возвращает указатель,
> и сам этот указатель — обычный локальный объект области видимости.
> Когда область заканчивается, указатель уничтожается,
> но объект в куче при этом никак не затрагивается.
...
> здесь мы делаем это намеренно, чтобы пронаблюдать мусорное значение.
...
> 🤓 «очистка» перемещаемой строки здесь — деталь реализации.
...
> 🤓 для именованной переменной вроде `demo1` ниже это деталь реализации (NRVO).
...
*lvalue* — это значение, которое может появиться в левой части присваивания.
Иначе говоря, *lvalue — значение, у которого есть хранилище*: оно хранится в какой-то памяти.
Другими словами, *у lvalues есть адрес в памяти*.

*rvalue* хранилища не имеет: это может быть временное значение или константа.
rvalues могут появляться в правой части присваивания.

<details>
<summary>Более точное определение (категории выражений)</summary>
...
> Поэтому определения выше — упрощения, которые ломаются на ссылках и `std::move`.
