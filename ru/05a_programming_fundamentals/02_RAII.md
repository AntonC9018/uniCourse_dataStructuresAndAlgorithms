...
В C++ у областей видимости две функции:
- Они делают переменные невидимыми за своими пределами;
- Они служат для очистки памяти («ресурсов») объектов.
  Это достигается с помощью деструкторов.

Можно сказать, что области видимости позволяют управлять временем жизни объектов.
...
Концептуально переменная, объявленная в области, уничтожается в конце области.
...
Деструктор — это специальная `void`-функция, которая вызывается в этом контексте автоматически.
...
*rvalue-ссылки* обозначаются `T&&` — это временный объект, значение которого не будет использоваться позже.
Заставить объект считаться rvalue-ссылкой можно с помощью `std::move`.
