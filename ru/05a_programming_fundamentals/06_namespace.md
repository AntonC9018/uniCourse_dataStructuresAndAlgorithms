...
А значит, открывать существующие пространства имен и дополнять их — можно.
Но пространство имен `std` разрешено дополнять только в явно разрешенных стандартом случаях,
например специализацией `std::hash` под ваш собственный тип.
Добавлять же в `std` произвольные сущности (вроде своей функции `std::f`) нельзя:
это неопределенное поведение — стандартная библиотека вправе считать,
что контролирует все содержимое `std`.
