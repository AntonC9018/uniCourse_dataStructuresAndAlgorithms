...
`auto` выводит тип по правилам вывода параметров шаблонов:
верхнеуровневые `const` и ссылки отбрасываются, а массивы и функции «распадаются» на указатели.
`decltype(y)` вместо этого сохраняет объявленный тип `y`.
Поэтому `auto x = y;` не всегда эквивалентно `decltype(y) x = y;` —
