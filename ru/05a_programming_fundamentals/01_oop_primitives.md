...
> **Ну вообще-то**: описание ссылки как «указателя, который нельзя переприсвоить» — деталь реализации.
...
struct Person
{
    ...
    // у нее нет неявного параметра `this`, то есть она не привязана к экземпляру.
...
int main()
{
    ...
    // `Person::printAge` — квалифицированное имя: `printAge` ищется в области видимости
    // класса `Person` (это область видимости класса, а не пространство имен).
    // Заметьте: `::` — не обязательное требование, а способ квалификации имени;
    // статическую функцию-член можно вызвать и через объект: `person.printAge(&person)`.
