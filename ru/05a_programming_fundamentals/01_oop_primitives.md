...
Сначала сделаем её статической функцией-членом, чтобы увидеть, как работает концепция *пространств имен*.
...
struct Person
{
    ...
    // Ключевое слово `static` на самом деле не делает функцию статической,
    // оно лишь требует вызывать эту функцию через оператор разрешения области видимости (::).
...
int main()
{
    ...
    // Идея в том, что мы можем связать операции,
    // работающие с `Person`, с пространством имен `Person` этой структуры.
