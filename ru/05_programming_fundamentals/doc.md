...
| Адрес | Инструкция                                                                                         |
|-------|----------------------------------------------------------------------------------------------------|
| 100   | Выделение места для `a` в стеке                                                                    |
| 101   | Запись 5 в `a`                                                                                     |
| 102   | Добавление 10 к `a`                                                                                |
| 103   | Возврат: чтение адреса, сохраненного ниже `a`, переход на него                                     |
| 104   | Запись адреса 106 в стеке                                                                          |
| 105   | Переход к адресу 100                                                                               |
| 106   | Возврат: чтение адреса в стеке (сохраненного функцией, которая могла вызвать эту), переход на него |

Адреса 100-103 соответствуют функции `f`, а адреса 104-106 - функции `g`.
...
Стек будет выглядеть примерно так, когда мы вызываем `g`:
